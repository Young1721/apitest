...
public class ApiContorller {
	...
	public ResponseEntity<String> apiCall(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		ApiDocument apidocument = apiUtil.getApiDocument(requestParamProperty.getName(),
				requestParamProperty.getPhoneNumber(), requestParamProperty.getEmail(), apiUrlProperty.getUrl());
		...
		String flattenedJson = JsonFlattener.flatten(jsonData);		
		...
		fw.write(flattenedJson);
		fw.flush();

		FTPClient ftpClient = apiUtil.setFTPUploader(fileProperty.getUploadIp(), fileProperty.getUploadUsername(),
				fileProperty.getUploadPassword());
